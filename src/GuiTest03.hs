...
-- Game Boy Emulator
--
-- This module runs is supposed to be a gtk+ application that emulates a ROM,
-- using the IO based emulation core. It doesn't seem to compile.
--
-----------------------------------------------------------------------------

module GuiTest03 where
  ...
  let
      ...
      refreshDisplay d = do
        ...
        doFromTo 0 143 $ \y ->
          let scanline = d!y in
          doFromTo 0 159 $ \x -> do
            color <- readArray scanline x
            let colorByte = (fromIntegral color) * 85
      ...
      step = do
        ...
        case s of
          ...
          Just machineState -> do
            ...
            updateMachineDisplayFrameIO machineState

            refreshDisplay (msDisplay machineState)
      ...
      open = do
        ...
        case response of
          ResponseOk -> do
            ...
            writeIORef state $ Just machineState
